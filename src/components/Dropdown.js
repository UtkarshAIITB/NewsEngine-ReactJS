...
const Dropdown = (props)=> {
    ...
    const death_valid = () =>{
        ...
        return String(props.data?.deaths);
    }

    const discharged_valid = ()=>{
        if(String(props.data?.discharged) === 'undefined'){
            return "Select State";
        }
        return String(props.data?.discharged);
    }

    const deathstoday_valid = ()=>{
        if(String(props.data?.deaths - props.data_prev?.deaths) === 'NaN'){
            return "Select State";
        }
        return String(props.data?.deaths - props.data_prev?.deaths);
    }
    ...
    useEffect(() => {
        document.title = `NewsEngine - Covid-19 | ${props.name}` 
